...
class LearnableScoringMLP(nn.Module, BaseRegressionScorer):
    ...
    def __init__(self, input_dim, hidden_dims=[64, 32], output_dim=1, 
                 ...
                 final_activation='softplus', l2_lambda=1e-5, min_score=0.01):
        """
    ...
    def score(self, inputs, targets=None, predictions=None):
        ...
        Args:
            ...
            predictions: Model predictions (unused in this scorer)
...
class ResidualScoringMLP(LearnableScoringMLP):
    ...
    """
    def __init__(self, feature_dim, hidden_dims=[64, 32], output_dim=1, 
                 dropout_rate=0.2, use_batch_norm=True, activation='leaky_relu',
                 final_activation='softplus', l2_lambda=1e-5, min_score=0.01):
        """
        ...
        Args:
            ...
            min_score: Minimum score to prevent degenerate intervals
        """
        # Input dimension is feature_dim + 1 (features + residual)
        super().__init__(
    ...
    def score(self, inputs, targets=None, predictions=None):
        ...
        combined_input = torch.cat([inputs, residuals], dim=1)
        
        # Score using the combined input
        with torch.no_grad():
            scores = self.forward(combined_input)
...
def get_scoring_function(config, feature_dim=None):
    ...
    if scorer_type == 'abs_residual':
        ...
        min_score = mlp_config.get('min_score', 0.01)
